...
from src.models.product import SKU, ProductStatus, ProductFieldReport
...
def moderation_worker():
    ...
    consumer_thread = threading.Thread(
        target=consumer_instance.start, 
        daemon=True
    )
...
class TestModerDecisionApply(BaseTestUtil):
    def test_moderated_event_clears_blocking_data(self, product_factory, moderation_worker):
        product = product_factory(status=ProductStatus.ON_MODERATION, blocking_reason="test")
    ...
    def test_moderation_blocking_descision(self, product_factory, moderation_worker, hard_blocked):
        ...
        for _ in range(10):
            product.refresh_from_db()
            if product.status == (ProductStatus.HARD_BLOCKED if hard_blocked else ProductStatus.BLOCKED):
                success = True
                break
            time.sleep(0.5)
    ...
    def test_duplicate_event_same_idempotency_key_no_side_effects(self, product_factory, moderation_worker):
        product = product_factory(status=ProductStatus.ON_MODERATION, blocking_reason="test")
        ProductFieldReport.objects.create(
            product=product,
            field="title",
